...
| **机器** | 长闭环/评测：**4090（cursor-125）**；训世界模型/策略：**H100**（**一律经 125 SSH 到 H100**，Mac 不直连）；Mac：文档与接线，不开长 GPU |
...
| 机 | 现在同步跑 |
|----|------------|
| **125** | B 诊断/重训调度；C2 已齐；**经 125 SSH 开 H100** |
| **H100** | coll 损失再训 WM；**勿**无 B 猛训 π |
| **Mac** | 文档/接线（若在线）；**不直连 H100**；不开长 GPU |
