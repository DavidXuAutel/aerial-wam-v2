...
class RealCameraConfig:
    ...
    # Apply v4l2 preset before open (None = skip). SJCAM has no v4l2 controls.
    v4l2_preset: Optional[str] = None
    v4l2_profile: str = "sjcam"  # "hj" | "c922" | "sjcam"
...
class RealCamera:
    ...
    def open(self) -> None:
        ...
        if self.config.v4l2_preset:
            profile = str(self.config.v4l2_profile).strip().lower()
            if profile == "c922":
                from experiments.aerial.deploy.c922_controls import apply_preset
            else:
                from experiments.aerial.deploy.hj_camera_controls import apply_preset
