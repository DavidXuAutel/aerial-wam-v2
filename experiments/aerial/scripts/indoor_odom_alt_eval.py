#!/usr/bin/env python3
"""Indoor Evaluation with Decoupled Altitude Control & Odometry Tracking (Full-Fix).

Uses RolloutCollector to ensure identical execution environment and mechanics as Step 1.
Adds custom Step Hooks to evaluate:
  1. Baseline Indoor (Step 1 micro-limits + micro-shield)
  2. Dual-Rate Altitude Lock (Geometric Z PD controller + Odometry Tracker)

Usage on 125:
  source experiments/aerial/scripts/env_4090.sh
  $AERIAL_PY experiments/aerial/scripts/indoor_odom_alt_eval.py \
    --config configs/aerial_rl_indoor_lossless.yaml \
    --episodes 6 \
    --out artifacts/indoor_odom_alt_eval_20260828.json
"""
...
logger = logging.getLogger("indoor_odom_eval")
...
class AltitudeLockedPolicyWrapper:
    """Wraps a WAM deploy policy and overrides dz with an AltitudeLockController."""

    def __init__(self, base_policy: Any, max_dz: float = 0.08, step_hz: float = 5.0):
        self.base_policy = base_policy
        self.max_dz = max_dz
        self.step_hz = step_hz
        from experiments.aerial.rl.indoor_controller import AltitudeLockController
        self.alt_ctrl = AltitudeLockController(kp=1.5, kd=0.6, max_dz=max_dz)
        self.target_z: Optional[float] = None

    def reset(self, target_z: Optional[float] = None):
    ...
    def act(self, view: Any) -> np.ndarray:
        act = self.base_policy.act(view)
        if self.target_z is not None:
            current_z = float(view.proprio[2])
            dz = self.alt_ctrl.step(current_z, dt=1.0 / self.step_hz)
            act[2] = dz
        return act
...
def run_evaluation_mode(
    ...
    enable_altitude_lock: bool = False,
    max_steps: int = 100,
    ...
    wrapped_policy = (
        AltitudeLockedPolicyWrapper(policy, max_dz=action_limits[2], step_hz=step_hz)
        if enable_altitude_lock
        else policy
    )
    ...
    logger.info(f"\n=======================================================")
    logger.info(f"🚀 Running Evaluation: [{mode_name}] (Altitude Lock: {enable_altitude_lock})")
    ...
    for seg in segments:
        ...
        target_z = float(goal_pos[2])
        d0 = seg["d0_m"]

        if enable_altitude_lock and isinstance(wrapped_policy, AltitudeLockedPolicyWrapper):
            wrapped_policy.reset(target_z=target_z)
        ...
        z_errs = [abs(tr.obs.position[2] - target_z) for tr in ep_trans]
        ...
        logger.info(
            ...
            f"z_err={mean_z_err:4.2f}m | prog={prog_ratio*100:+5.1f}% | arrived={arrived!s:5s} | shield={interv_rate*100:4.1f}%"
        )
        ...
        results.append({
            ...
            "mean_z_error_m": round(mean_z_err, 3),
    ...
    summary = {
        ...
        "mean_z_error_m": round(float(np.mean(z_variations)), 3) if z_variations else 0.0,
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Indoor Dual-Rate Altitude & Odometry Evaluation")
    ...
    parser.add_argument("--out", default="artifacts/indoor_odom_alt_eval_20260828.json")
    ...
    # 1. Mode A: Without Altitude Lock (Baseline from Step 1)
    summary_baseline = run_evaluation_mode(
        mode_name="Indoor_Micro_Baseline",
        segments=segments,
        env=env,
        policy=policy,
        dynamics=dynamics,
        ...
        reward_cfg=reward_cfg,
        action_limits=indoor_limits,
        enable_altitude_lock=False,
        max_steps=args.max_steps,
        success_dist=args.success_dist,
    )

    # 2. Mode B: With Altitude Lock & Odometry Tracking (Step 2)
    summary_alt_locked = run_evaluation_mode(
        mode_name="Indoor_Micro_DualRate_AltitudeLock",
        segments=segments,
        env=env,
        policy=policy,
        dynamics=dynamics,
        ...
        reward_cfg=reward_cfg,
        action_limits=indoor_limits,
        enable_altitude_lock=True,
        max_steps=args.max_steps,
        success_dist=args.success_dist,
    )

    out_payload = {
        "evaluation_title": "Indoor Dual-Rate Altitude Lock & Odometry Tracking Evaluation",
        "timestamp": time.strftime("%Y-%m-%d %H:%M:%S"),
        "target_precision_m": args.success_dist,
        "segment_count": len(segments),
        "comparison": {
            "indoor_micro_baseline": summary_baseline,
            "indoor_micro_altitude_locked": summary_alt_locked,
        },
    }
    ...
    logger.info(f"\n=======================================================")
    logger.info(f"📊 Dual-Rate Altitude Lock Evaluation Complete!")
    logger.info(f"   Baseline Arrival: {summary_baseline['arrival_rate']*100:.1f}%, Mean Final Dist: {summary_baseline['mean_final_distance_m']:.2f}m, Mean Z-Err: {summary_baseline['mean_z_error_m']:.2f}m")
    logger.info(f"   Alt-Lock Arrival: {summary_alt_locked['arrival_rate']*100:.1f}%, Mean Final Dist: {summary_alt_locked['mean_final_distance_m']:.2f}m, Mean Z-Err: {summary_alt_locked['mean_z_error_m']:.2f}m")
    logger.info(f"=======================================================")
    return 0
