...
| M5e | **C2 有界策略分布**（动作空间一致性） | 🟡 **代码已落地 + Mac 数学/回归已验**；⏳ §A 重跑 → 从零重训 → ① 再 gate（`n≥8`） |
