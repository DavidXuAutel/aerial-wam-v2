...
**⚠️ 「§4 充分」尚不成立**：A.2 判据只比两个**单位幅度**常量，探测不到**幅度通道**。实测 π 想象 λG0 **+104.72**（最高）却真实 ① **−8.74**（最低；前飞 +47.02 / heur +8.42）= **排序倒挂**，即想象目标被薅（命门 A）。机制为幅度：`a0` 模长 ≈3.36（`action_scale=3` 饱和），Σprogress +81.92 只换来 Σmaneuver −2.32 ⇒ 约 **35× 欠罚**，顶满 ‖a‖ 即想象最优、方向次要。**下一件 = §A.3 幅度匹配臂（只读）**，判据见提案 §A.3；A.3 前不签「§4 充分」。  
