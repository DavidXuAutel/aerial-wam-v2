...
**下一件 = §A.4（只读，`--clip-actions`）**：把三臂夹进部署动作集合再判方向。`(b)≥(a)` → 倒挂全来自无界动作通道，先落动作空间一致性并重跑 §A；`(a)>(b)` → 才确诊 RH 案。  
**处置**：In 表修订仍在提案里但 **当前不充分**；**不**开 RH 案（依据已撤）。不加长训、不降 `δ_p`、不翻 yaml。待签字稿：[V4_SIGNAL1_STRUCTURAL_REFREEZE_PROPOSAL](V4_SIGNAL1_STRUCTURAL_REFREEZE_PROPOSAL.md)。  
