...
**✅ 动作空间一致性裁定 = C2（2026-08-18 签字，代码已落地 + 从零重训 + ① 再 gate）**：`n_action_clipped=0`（fresh 与训后 π 均是）；训后想象 π **前向** a0 x=+0.567，λG0 59.09 > 最大前飞 47.78。① 两跑均 **FAIL**（−7.43 / −3.53 vs heur ~9，**n=5 < 8** 非全权）；④ **PASS** 0.000 vs 0.400。判定 **`clip_insufficient`**。下一件 = 签 §4 In 表。yaml 未翻。  
**✅ 动作空间一致性裁定 = C2（2026-08-18 签字，代码已落地）**：策略分布改为 `u~N(mean,std)`、`a = limits ⊙ tanh(u)`（`limits = body_delta_limits(1/env.step_hz)` = 部署同一盒），logp 加 tanh 雅可比 ⇒ 采样律**结构性**落在部署动作集内、REINFORCE 无偏。~~**下一件 = 重跑 §A/§A.3/§A.4（`n_action_clipped` 应为 0）→ H100 从零重训（禁 warm-start 现 ckpt）→ 125 ① 再 gate（`n≥8`，判据 `clip_helped`/`clip_insufficient`/`spurious_pass` 已事前写死于提案 §4.1）**。~~ ← **已做，见上句。** 下段保留裁定当时的推理原文：  
