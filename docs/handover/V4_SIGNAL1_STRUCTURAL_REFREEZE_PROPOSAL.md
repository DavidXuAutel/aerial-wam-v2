...
> **签字前置**：§A **已跑**（2026-08-18）→ A.2 = **`(b)>(c)`**（想象不偏好后退）。§4 按该判据为**充分**；仍待签字栏其余项。
...
- [x] **先做 §A 只读诊断**（第一件事）；A.2 判定结果 = **`(b)>(c)`**（λG0 47.02 vs 15.90；差额 = progress，p_coll≈0）→ §4 **充分**。数字：[`V4_SIGNAL1_SA_DIAG_STATUS.md`](V4_SIGNAL1_SA_DIAG_STATUS.md)
