...
| 2b | [`V4_SIGNAL1_STRUCTURAL_REFREEZE_PROPOSAL.md`](V4_SIGNAL1_STRUCTURAL_REFREEZE_PROPOSAL.md) | **待签字**；**下一件 = §A 只读诊断**（是否想象偏好后退） |
...
- 「V4-① 再训一轮 / 对齐 z0 就能过」—— **否**；现 In 表 `π(a|z)` 对 heuristic **不可稳健达成**。烙印解释恒定、不解释负号。下一件是提案 **§A**。见 [结构提案](V4_SIGNAL1_STRUCTURAL_REFREEZE_PROPOSAL.md)。
