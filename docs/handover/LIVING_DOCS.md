...
| 2d | [`V4_C2_COS_DIAG_125_STATUS.md`](V4_C2_COS_DIAG_125_STATUS.md) | **DONE**：C2 ①-eval cos **+0.806 / +0.762** ⇒ **不签** §4 In 表（想象-真实倒挂） |
...
- **「那个 cos 已在 C2 re-gate 产物里」—— 否**（08-18）。gate partial-1 只落 progress / final_dist / scan（`v4_gate_run_partials.py:228-234`），**无 cos 无动作**。出 cos 的是 `v4_progress_diag.py`（M5d 跑过 ≈−0.88；C2 diag 跑过 **+0.76~+0.81**）。
...
2. [`V4_C2_COS_DIAG_125_STATUS.md`](V4_C2_COS_DIAG_125_STATUS.md) — C2 cos diag **DONE**：mean cos **+0.806/+0.762** ⇒ **不签** §4；下一件 = 想象-真实倒挂另案（非 In 表）  
3. `ACCESS.md`  
