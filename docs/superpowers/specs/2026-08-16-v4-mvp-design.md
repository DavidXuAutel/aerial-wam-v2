...
| Actor | `act_latent(z)`（或 `h‖z`）；输出 4-D 运动学动作。**2026-08-18 记载**：此接口 **看不见 goal**（非实现 bug）。在 ① vs Heuristic 下 **不可稳健达成**（非数学恒负；见提案 §3）。签字前置 **§A 只读诊断** — [`V4_SIGNAL1_STRUCTURAL_REFREEZE_PROPOSAL.md`](../../handover/V4_SIGNAL1_STRUCTURAL_REFREEZE_PROPOSAL.md)；签字前 **禁止** 现 π 加长训 |
...
- **2026-08-18** — 记载：In 表 goal-blind + 单 mock goal 使 ① **不可稳健达成**（非数学恒负）。方向为负待 §A。修订待签字，**本文件 In 表尚未改**。
